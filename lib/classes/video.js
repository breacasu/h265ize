...
class Video {
    ...
    filesystem() {
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (fs.existsSync(_self.output.path) && !_self.options.override) {
    ...
    autoCrop() {
        let _self = this;
        return new Promise(function(resolve, reject) {
            if (_self.options.autocropOff) {
                _self.encoder.logger.debug('Autocrop disabled.');
                return resolve();
            }
        ...
        return new Promise(function(resolve, reject) {
            ...
            _.each(_self.streams.audioStreams, function(stream, i) {
                ...
                if (!(audioTitle) && _self.options.normalizeLevel >= 2) {
                    ...
                    audioTitle = normalizedLanguage + ((stream.profile && stream.profile !== 'unknown') ? (' ' + stream.profile) : '') + ' (' + channelsFormated + ')';
                    _self.encoder.logger.alert('Audio does not have a title. Title set to', '"' + audioTitle + '".');
                    _self.ffmpegCommand.outputOptions('-metadata:s:a:' + i, 'title="' + audioTitle + '"');
                }
            ...
            _.each(_self.streams.audioStreams, function(stream, i) {
                if (stream.codec_name !== 'flac' || _self.options.forceHeAudio) {
                    ...
                    if (!(audioTitle) && _self.options.normalizeLevel >= 2) {
                        ...
                        _self.encoder.logger.alert('Audio does not have a title. Title set to', '"' + audioTitle + '".');
                        _self.ffmpegCommand.outputOptions('-metadata:s:a:' + i, 'title="' + audioTitle + '"');
                        // Fix: see https://trac.ffmpeg.org/ticket/5718
                        _self.ffmpegCommand.outputOptions('-af aformat=channel_layouts="' + channelsFormated + '"');
                    }
                } else {
    ...
    setTargetDate() {
        let _self = this;
        return new Promise(function(resolve, reject) {
            if (!_self.options.keepDate)
                return resolve();

            // set file add/mode time
            var stats = fs.statSync(_self.path);
            // _self.encoder.logger.debug('Video data:', stats);
            var mtime = new Date(stats.mtime);
            // _self.encoder.logger.debug('Video mtime: ', mtime);
            _self.encoder.logger.debug('Setting date of encoded file to ', mtime);
            fs.utimesSync(_self.output.path, mtime, mtime);
            resolve();
    ...
    start() {
        ...
        this.encoder.logger.verbose('Encoding started at', colors.yellow(this.startTime.format("ddd, H:mm")));
        this.running = true;
